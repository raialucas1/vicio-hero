...
const STORAGE_PREFIX = "vicioHero:state:";
...
function loadStateByEmail(email, nome) {
...
async function saveState() {
  if (state.usuario?.email) {
    localStorage.setItem(getStorageKey(state.usuario.email), JSON.stringify(state));
  }
...
async function login() {
  ...
  state = loadStateByEmail(email, nome);
  state.usuario = { ...state.usuario, nome, email };
  saveState();
}
